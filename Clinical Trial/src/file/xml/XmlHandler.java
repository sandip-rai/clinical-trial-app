...
public class XmlHandler extends Handler {
	...
	public boolean readXMLFile(String file) {
		...
		try {
			//Instantiate a new JAXBContect & Unmarshaller to read the XML
			JAXBContext jaxbContext = JAXBContext.newInstance(ReadingSet.class);
			Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
			//Create a ReadingSet object from the XML file using the Unmarshaller
			ReadingSet readingSet = (ReadingSet) jaxbUnmarshaller.unmarshal(new File(file));
			//Convert from a reading set to an ArrayList of FileReadings so that the data can be processed
	 ...
	 *
	 * @param readingSet the reading set
